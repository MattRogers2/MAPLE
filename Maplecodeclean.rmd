...
source("limmaphyloseq.r")
library(magrittr)
library("graph4lg")
source("miseqR.R")
source("ancom_updated_code.r")
source("sanitizefunctions.r")
library("microbiome")
library("Maaslin2")
...
ps<-import_biom("Maple.biom",treefilename="tree.nwk",refseqfilename="dna-sequences.fasta",parseFunction=parse_taxonomy_greengenes)
tree<-phy_tree(ps)
...
metadata<-read.xlsx("MapleFinalMeta.xlsx")
...
metaABX<-read.xlsx("MAPLE_ABX.xlsx",sheet=4)
...
metaoutcomes<-read.xlsx("MapleFinalMeta.xlsx",sheet=5)
...
LRTIdiagnosis<-read.xlsx("MapleFinalMeta.xlsx",sheet=3)
...
rns<-gsub("ASAL-","",rns)
rns<-gsub("-NA-OW","",rns)
metaDRDR<-read.xlsx("DRDR_Healthy_Controls_Self_Report.xlsx")
...
metadata<-read.table("DRDRmeta.txt",sep="\t",header=T)
metadata<-metadata[,-1]
...
FecalsamplesvsSalivaplotAge<-ggplot(Alldists,aes(x=Group,y=Wunifrac_Distance_Means))+geom_boxplot()+ggtitle("Distribution of Average Distances of Fecal samples to LRTI Saliva Samples by Age Group")
...
ggsave(file="/Users/rogersm/MapleFigs/Totallyfinalfiles/Sendout/BMCSubmit/Resubmission/BMCresubmit14042022/LRTIFecaltoSalivaDistancesbyAge.pdf",plot=FecalsamplesvsSalivaplotAge,width=8.5,height=11)
...
#grid.arrange(plotlist[[2]],plotlist[[3]],nrow=1)
#dev.off()
```
...
#grid.arrange(pl[["COPD_Hx"]],pl[["OUTCOME_COPD"]],legend,nrow=2)
#dev.off()
...
metasforcorr<-read.csv(metas.csv")
...
tests<-as.list("")
gplotlist<-as.list("")
psageadjusted2<-psageadjusted
ps_rare<-rarefy_even_depth(psageadjusted2,rng=9999)
psall_time<-subset_samples(ps_rare, Site.y=="O")
Allsamplesunifrac<-UniFrac(psall_time, weighted=TRUE)
set.seed(4235421)
meta<-meta(psall_time)
metas<-sanitizerReference(meta)
rownames(metas)<-metas[,1]
sample_data(psall_time)<-metas
metadata<-meta(psall_time)
psall_asthma<-subset_samples(psall_time, (Batch=="1"&Site.y=="O"&Batch=="1"&Site.x!="NA"&Asthma_Hx=="Yes")|(Batch=="Reference"))
...
bdiv<-cbind(meta, coordPCA)
bdiv<-as.data.frame(bdiv,stringsAsFactors=FALSE)
pcoaAsthma<-ggplot(bdiv,aes(x=Axis.1,y=Axis.2,colour=Batch))+geom_point()+ggtitle("Asthma PCoA")+xlab("PcoA1(25.6%)")+ylab("PCoA2(15.7%)")+xlim(-0.03,0.05)

adbatch<-adonis2(Allsamplesunifrac~Batch, data=meta)
adcity<-adonis2(Allsamplesunifrac~city, data=meta)
kable(ad2,digits=2)
j<-pairwise.adonis2(Allsamplesunifrac~city,data=meta,nperm=9999)
length(j)
for (i in 2:length(j)){
  comp<-names(j)[i]
  R<-j[[i]]$R2[1]
...
#meta<-sanitizerReference(psageadjusted)
#rownames(meta)<-meta[,1]
meta<-meta(psageadjusted2)
metas<-sanitizerReference(meta)
rownames(metas)<-metas[,1]
sample_data(psageadjusted2)<-metas
ps_asthma<-subset_samples(psageadjusted2, (Batch=="1"&Site.y=="O"&Batch=="1"&Site.x!="NA"&Asthma_Hx=="Yes"&Time==1)|(Batch=="Reference"))

meta_data<-meta(ps_asthma)
psofamily<-ps_asthma %>%
  tax_glom(taxrank = "Family") %>%                     # agglomerate at phylum level
  transform_sample_counts(function(x) {x/1})
psofamilyrelative<-ps_asthma %>%
  tax_glom(taxrank = "Family") %>%                     # agglomerate at phylum level
  transform_sample_counts(function(x) {x/sum(x)})
taxlist<-tax_table(psofamily)
taxlist<-taxlist[,5]
...
res<-""
res2<-""
feature.table = otu_absolute; sample.var = "Sample_ID"; group.var="Batch"; 
zero.cut = 0.90; lib.cut = 1000; neg.lb = TRUE
pre.process = feature_table_pre_process(feature.table, meta_data, sample.var, 
                                        group.var, zero.cut, lib.cut, neg.lb)
feature.table = pre.process$feature.table
...
res2$q.val[res2$q.val==0.000]<-0.001
melted<-melt(res2, id.vars=c("taxon","W","p.val","q.val","diff.abn","otu_means","label","type"))
melted$label<-as.vector(melted$label)
asthmaplot<-ggplot(melted, aes(x=value, y=-log(q.val),label=label,colour=type))+geom_point(aes(size=otu_means,alpha=0.5))+geom_text_repel(size=3,segment.size =0.2)+facet_wrap(~variable,ncol=3)+scale_color_manual(values=c("red","blue","grey"))+geom_vline(xintercept = 0,linetype=2)+theme_classic()+xlim(-7,7)+ggtitle("Asthma taxon comparisons")
...
psageadjusted2<-psageadjusted
ps_rare<-rarefy_even_depth(psageadjusted2,rng=9999)
psall_time<-subset_samples(ps_rare, Site.y=="O"|Batch=="Reference")
metas<-meta(psall_time)
metas<-sanitizerReference(metas)
rownames(metas)<-metas[,1]
sample_data(psall_time)<-metas
#metadata<-meta(psall_time)
psall_copd<-subset_samples(psall_time, (Batch=="1"&Site.y=="O"&Batch=="1"&COPD_Hx=="Yes")|(Batch=="Reference"))
...
bdiv<-cbind(meta, coordPCA)
bdiv<-as.data.frame(bdiv,stringsAsFactors=FALSE)
pcoacopd<-ggplot(bdiv,aes(x=Axis.1,y=Axis.2,colour=Batch))+geom_point()+ggtitle("COPD PCoA ")+xlim(-0.03,0.05)+xlab("PCoA1(26.1%)")+ylab("PCoA2(14.1%)")
adbatch<-adonis2(Allsamplesunifrac~Batch, data=meta)
adcity<-adonis2(Allsamplesunifrac~city, data=meta)
kable(ad2,digits=2)
j<-pairwise.adonis2(Allsamplesunifrac~city,data=meta,nperm=9999)
...
metas<-meta(psageadjusted2)
metas<-sanitizerReference(metas)
rownames(metas)<-metas[,1]
sample_data(psageadjusted2)<-metas
ps_copd<-subset_samples(psageadjusted2, (Batch=="1"&Site.y=="O"&Batch=="1"&Site.x!="NA"&COPD_Hx=="Yes"&Time==1)|(Batch=="Reference"))

meta_data<-meta(ps_copd)
psofamily<-ps_copd %>%
  tax_glom(taxrank = "Family") %>%                     # agglomerate at phylum level
  transform_sample_counts(function(x) {x/1})
psofamilyrelative<-ps_copd %>%
  tax_glom(taxrank = "Family") %>%                     # agglomerate at phylum level
  transform_sample_counts(function(x) {x/sum(x)})
taxlist<-tax_table(psofamily)
taxlist<-taxlist[,5]
...
melted<-melt(res2, id.vars=c("taxon","W","p.val","q.val","diff.abn","otu_means","label","type"))
melted$label<-as.vector(melted$label)
copdplot<-ggplot(melted, aes(x=value, y=-log(q.val),label=label,colour=type))+geom_point(aes(size=otu_means,alpha=0.5))+geom_text_repel(size=3,segment.size =0.2)+facet_wrap(~variable,ncol=3)+scale_color_manual(values=c("red","blue","grey"))+geom_vline(xintercept = 0,linetype=2)+theme_classic()+xlim(-7,7)+ggtitle("COPD taxon comparisons")
grid.arrange(pcoaAsthma,pcoacopd,asthmaplot,copdplot,nrow=2)
```
...
#PcoA plots of significant variables Fecal samplea
```{r}
